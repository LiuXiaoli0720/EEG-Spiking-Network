...
def to_alpha(data, window, args):
    ...
    for window_index in tqdm(range(window.shape[0])):
        ...
        alpha_data.append(data[start:end, :])
    alpha_data = np.stack(alpha_data, axis=0)
    return alpha_data

def main(name="S1", data_document_path="/root/KUL_single_single_snn_1to32_mean"):
    args = DotMap()
    args.name = name
    ...
    args.ConType = ["No"]
    args.fs = 128
    args.window_length = math.ceil(args.fs * 0.5)
    args.overlap = 0.5
    args.batch_size = 32
    args.max_epoch = 200
    ...
    np.savez(f"{makePath(f'/root/autodl-tmp/EEG/dataset/{args.window_length//128}s')}/{name}",
